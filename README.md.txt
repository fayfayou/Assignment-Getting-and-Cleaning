#first line of code set the working directory to "UCI HAR Dataset"
#second step was to read all datasets downloaded for this assignment including: X_train, X_test and their corresponding label activities  using those lines of code:
	train_set <- read.table("train/X_train.txt")
	test_set <- read.table("test/X_test.txt")
	activity_train <- read.table("train/y_train.txt")
	activity_test <- read.table("test/y_test.txt")
#we combined the two datasets of activities labels corresponding to trainin set and test set using rbind() function, then we changed the variable name to "activiy_label"
	activity <- rbind(activity_train, activity_test)
	activity <- rename(activity, activity_label = V1)
#we named the activities with their descriptive names using those lines of code: 
...
#we called the datasets containing information about the subjects, the we combined them using rbind() and rename the variable name to "subject" : 
	subject_train <- read.table("train/subject_train.txt")
	subject_test <- read.table("test/subject_test.txt")
	subject <- rbind(subject_train, subject_test)
	subject <- rename(subject, subject = V1)
#we combined the datasets X_train and X_test in the same way as before, we select the columns corresponding to the mean and standard deviation of all the measurements and we change their variable names : 
...
#we calculate, for each activity and each subject, the average of all the variables :
	seconddataset <- dataset %>% group_by(activity_label, subject) %>% summarise_all("mean")
#finally, we store the output tidy data in the format of text document:
